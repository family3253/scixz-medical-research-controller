...
Evidence-anchored overall assessment, major/minor concerns, editorial recommendation, cross-review consensus, uncertainty, and revision tasks. Any PaperReview.ai result is retained as an `external-signal` and converted into a separately labelled, evidence-anchored issue ledger with stable `PR-xx` identifiers. The final synthesis must record exactly one disposition for every external issue and include a cross-branch agreement/disagreement matrix. When bilingual Word output is requested, the primary deliverable is one Chinese and one English full editorial report rendered through `scripts/render_editorial_review_docx.py --structure-profile ... --fusion-bundle ...`. It must contain materials reviewed, editorial decision, reviewer panel, overall assessment and strengths, P0/P1/P2/P3 findings, issue sources, required actions, acceptance criteria, adversarial stress test, author questions, dimension scores, and a phased revision roadmap. The strict fusion audit remains in appendices. `scripts/render_final_review_docx.py` may additionally produce a compact machine-audit attachment, but it must not replace the full report. Strict mode blocks omitted, duplicated, or unknown issue IDs. Formal `nature-review-studio` output is exactly one DOCX plus one same-stem Markdown file.
...
6. Run Critic, Consensus/政事堂, and Verifier on the fused output, then write the bilingual final-review JSON plus the completed editorial structure profile and render the full Chinese/English DOCX reports in strict fusion mode. Verify the visible section inventory before release.
