#!/usr/bin/env python3
"""Unified CLI for the cycppt skill."""
...
def cmd_add_page(args: argparse.Namespace) -> int:
    argv = [args.run, args.image]
    if args.source_page is not None:
        argv.extend(["--source-page", str(args.source_page)])
    return run_script("add_page_to_run.py", argv)
...
""",
  ...
  editppt prepare a.png b.png --out-root output/cycppt
""",
    )
...
""",
    ...
    add_page = run_sub.add_parser(
        "add-page",
        help="Append one generated slide image to an existing run.",
        description="Append a generated slide image as the next pending page in an existing editppt run. Use this for streaming medical workflows where slide images become ready one at a time.",
        formatter_class=HELP_FORMATTER,
    )
    add_page.add_argument("run", metavar="RUN", help="Run directory or deck_manifest.json path.")
    add_page.add_argument("image", metavar="IMAGE", help="Generated slide image to append.")
    add_page.add_argument("--source-page", type=int, metavar="N", help="Original/generated slide number. Defaults to the next page index.")
    ...
    finalize = run_sub.add_parser(
        "finalize",
        help="Build and validate the final editable and image-only decks.",
        description="Build final/<origin>_edited.pptx from recorded page manifests, build final/<origin>_image_only.pptx from slide images, and write validation outputs.",
        formatter_class=HELP_FORMATTER,
    )
