...
def slide_size_type(width, height):
    # ST_SlideSizeType uses the OOXML token ``screen16x9``.  ``wide`` is a
    # convenient internal label used by editppt manifests, but it is not a
    # valid value for p:sldSz/@type and causes desktop PowerPoint to reject
...
def theme_xml():
    """Return a compact schema-valid DrawingML theme.

    DrawingML requires exactly three entries in each format-style list.  A
    one-entry list can be parsed by some libraries but is rejected by desktop
    PowerPoint and by the Open XML SDK validator.
    """
    return """<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
