...
2. Execute the three steps in order: (1) background recognition and repair, (2) foreground asset separation, (3) native element reconstruction. Start the PaddleOCR/text-hints pass immediately after reading the required references, in parallel with foreground asset separation when possible, but do not consume final text placement before object-source decisions are recorded.
...
1. Start or verify the PaddleOCR/text-hints pass immediately. If `text_hints.json` is missing or the parent has just configured a PaddleOCR token, run `editppt page hints {{PAGE_DIR}}`; otherwise verify the existing hints and continue. Do not wait for foreground image jobs to finish before starting OCR.
2. Build the page inventory (Pre-Decision Checklist in page-decision-tree.md).
3. Decide the background (page-decision-tree.md section 1) and record `background_strategy`.
4. Decide and separate foreground assets (section 2). For evidence images that came from source material original files, prefer the original material file as the `editppt image edit --image` input or as the evidence image placement source when the plan requires the original figure. This is not a direct crop shortcut: it must still be recorded through asset-sheet separation or evidence-image placement with `asset_provenance`.
5. Submit all step-1/2 image jobs (clean bases and asset sheets) as one `editppt image batch` call while OCR/text-hints are already available or running, then record and process the results with `editppt image import` and `editppt image process-sheet`.
6. Rebuild native text, shapes, and tables (section 3). Fill `text_boxes` from the measured text hints per section 3.1; render formulas with `editppt formula render-latex` per section 3.2.
7. Write manifest.json following the field contracts in manifest-schema.md, including `text_inventory`, `visual_inventory`, `background_strategy`, `quality_checks`, and positioned `text_boxes`/`images`/`shapes`.
8. Build the artifacts with the deterministic runtime: `editppt page build {{PAGE_DIR}}` (writes page.pptx and preview.png from manifest.json), then `editppt page contact-sheet {{PAGE_DIR}}`, then `editppt page validate {{PAGE_DIR}}` — it runs the same manifest-contract checks `editppt run record` will run, so fix every reported issue here, inside the page.
