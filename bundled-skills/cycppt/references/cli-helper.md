...
|   |-- add-page                - append one generated slide image to an existing run as a pending page
...
```bash
python <skill-root>/scripts/validate-slide-prompt.py --prompt <task>/prompts/slide01.txt --slide-number 1
```

Purpose: validate that a full-authoring slide prompt is a rich GPT Image 2 generation prompt before dispatching a 页面生成 worker. It rejects summary-style prompts, missing expanded style-key text, missing slide-plan content, missing layout geometry, missing evidence/asset binding instructions, missing negative constraints, or missing continuity rules.
...
`<skill-root>` is the `cycppt` directory that contains `SKILL.md`. On Windows, use the same directory's `cli` subdirectory path.
...
`editppt doctor` reports the current text-hints backend; without a token everything still works through the built-in offline detector. Ask the user about a PaddleOCR-VL token only when OCR quality is blocking page reconstruction; after configuring a token, regenerate hints with `editppt run hints <run>`.
...
In the `FULL_AUTHORING_FROM_SOURCE` branch, prompt and slide-image gates are mandatory. Before dispatching a slide image worker, run `scripts/validate-slide-prompt.py` on `prompts/slideNN.txt`. After image generation, run `scripts/validate-slide-ready.py` against the real generated `slides/slideNN.png` and require `status: "IMAGE_READY"`, `passed: true`, `editppt image generate/edit/batch` provenance, and nonblank 16:9 content. Do not call `editppt prepare` to initialize a run from blank or white placeholder PNGs, `slides/sources` images, any generated slide that lacks `slide_results/slideNN.json`, or any slide prompt that failed the rich prompt gate.
...
Purpose: return a dispatched or recorded page to `pending`, clearing its dispatch and result records, so a new worker can be dispatched. Use it when a worker returned a failed page, `run record` rejected the outputs, or a dispatched worker is lost. First inspect the failure, fix the root cause, then reset and dispatch a fresh worker.

```bash
editppt run add-page <run> slides/slide02.png --source-page 2
```

Purpose: append one generated slide image to an existing run as the next pending page. Use this in the medical integrated workflow when slide images become ready one at a time and downstream page conversion should begin before every slide image has finished. The command copies the image into `input/`, creates `pages/page_NNN/source.png`, writes `page_request.json` and `imagegen-jobs.json`, appends `deck_manifest.json.pages`, and appends a pending entry to `page_jobs.json`.
...
`editppt run finalize` writes both required deliverables: final editable PPTX (`editable_output`) and final image-only PPTX (`image_only_output`). Both use `notes_manifest.json` so speaker notes are carried into each deck when available.
