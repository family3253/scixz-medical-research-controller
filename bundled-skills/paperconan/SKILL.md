...
## SciXZ runtime binding

When invoked through SciXZ on this machine, use the registered executable at:
`<CODEX_HOME>\runtimes\paperconan\Scripts\paperconan.exe`.
The user-level wrapper is `<CODEX_HOME>\bin\paperconan.cmd`. If `paperconan` is not visible in the current shell, use the absolute binding path or restart the shell so the updated user PATH is inherited.
