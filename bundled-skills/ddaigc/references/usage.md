...
- `prompts/ddaigc1.md`：第 1 轮改写提示词。
- `prompts/ddaigc2.md`：第 2 轮改写提示词。
- `prompts/ddaigc3.md`：第 3 轮改写提示词。
...
推荐优先使用 `scripts/run_aigc_round.py` 进行单轮执行，由脚本统一负责切块、还原和记录更新。
...
当前仓库同时支持两种入口，用户可自行选择：

1. 对话 skill 模式：直接在聊天框中触发降 AIGC skill，由对话按 `SKILL.md` 的规则执行单轮分段改写。这种方式不需要你自己配置模型 API。
2. 脚本 API 模式：使用 `scripts/run_aigc_round.py` 执行单轮分段处理；提供 OpenAI 兼容模型 API 配置后，脚本会逐块调用模型自动改写；如果不提供 API 配置，脚本默认直接报错，只有显式使用 `--dry-run` 时才只执行切块与还原校验。
