...
You can import this module from other Python code, or use the CLI:
  ...
  python scripts/aigc_records.py update-round \
      origin/xxx.txt 1 prompts/ddaigc1.md \
...
# Skill assets live relative to this file, but user data should live in the active workspace.
...
def get_finish_dir() -> Path:
    return get_workspace_root() / "finish"

def get_records_path() -> Path:
    return get_finish_dir() / "aigc_records.json"
...
def _ensure_finish_dir() -> None:
    get_finish_dir().mkdir(parents=True, exist_ok=True)
...
def save_records(records: Dict[str, Any]) -> None:
    ...
    get_records_path().write_text(text, encoding="utf-8")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    update_parser.add_argument(
        "prompt",
        help="Prompt file path used for this round (e.g. prompts/ddaigc1.md).",
    )
    update_parser.add_argument(
        "input_path",
