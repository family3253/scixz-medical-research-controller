...
def build_manifest(text: str, chunk_limit: int = DEFAULT_CHUNK_LIMIT) -> ChunkManifest:
    paragraphs = split_text_to_paragraphs(text)
    manifest_paragraphs: list[ParagraphManifest] = []
    manifest_chunks: list[Chunk] = []

    for paragraph_index, paragraph in enumerate(paragraphs):
        chunk_texts = split_paragraph_to_chunks(paragraph, chunk_limit)
        chunk_ids: list[str] = []
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "build":
        ...
        manifest = build_manifest(text, chunk_limit=args.chunk_limit)
