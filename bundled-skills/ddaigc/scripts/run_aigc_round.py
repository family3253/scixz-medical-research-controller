...
if TYPE_CHECKING:
    ...
    llm_module = import_module(f"{package_prefix}llm_client" or "llm_client")
    DEFAULT_CHUNK_LIMIT = cast(int, chunking_module.DEFAULT_CHUNK_LIMIT)
    build_prompt_input = cast(Callable[[str, str, int, str], str], round_module.build_prompt_input)
    load_prompt = cast(Callable[[int], str], round_module.load_prompt)
    run_round = cast(Callable[..., dict[str, object]], round_module.run_round)
    chat_completion = cast(Callable[..., str], llm_module.chat_completion)
    read_api_config = cast(Callable[[str | None, str | None, str | None], tuple[str | None, str | None, str | None]], llm_module.read_api_config)
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if resolved_api_key and resolved_model and resolved_base_url:
        ...
        def base_transform(chunk_text: str, prompt_input: str, round_number: int, chunk_id: str, /) -> str:
            return chunk_text
    else:
