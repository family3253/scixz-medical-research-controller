...
if TYPE_CHECKING:
    from .aigc_records import SKILL_DIR, get_workspace_root, update_round
    ...
    chunking_module = import_module(f"{package_prefix}chunking" or "chunking")
    SKILL_DIR = cast(Path, records_module.SKILL_DIR)
    get_workspace_root = cast(Callable[[], Path], records_module.get_workspace_root)
    update_round = cast(Callable[..., dict[str, object]], records_module.update_round)
    DEFAULT_CHUNK_LIMIT = cast(int, chunking_module.DEFAULT_CHUNK_LIMIT)
    build_manifest = cast(Callable[..., object], chunking_module.build_manifest)
    restore_text_from_chunks = cast(Callable[..., str], chunking_module.restore_text_from_chunks)
    save_manifest = cast(Callable[..., None], chunking_module.save_manifest)

PROMPTS = {
    1: "prompts/ddaigc1.md",
    2: "prompts/ddaigc2.md",
    3: "prompts/ddaigc3.md",
}
...
def relative_to_root(path: Path) -> str:
    ...
    return str(normalized.relative_to(get_workspace_root())).replace("\\", "/")


def load_prompt(round_number: int) -> str:
    prompt_path = SKILL_DIR / PROMPTS[round_number]
