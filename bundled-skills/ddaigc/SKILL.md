---
name: ddaigc
description: '对中文论文、摘要、课程作业、开题报告、实验报告、技术文档进行多轮降 AIGC / 去 AI 味改写。用户提到降AIGC、去AI味、降低AI痕迹、人性化改写、减少模板感和机器味、按顺序逐轮改写同一篇中文论文或技术文档时，必须使用本 skill；尤其当用户强调保留术语、数据、引用、段落结构、编号结构，或要求基于 origin/ 与 finish/aigc_records.json 在多个对话中继续下一轮时，应优先触发。本 skill 严格按 1-2-3 顺序依次使用 prompts/ddaigc1.md、prompts/ddaigc2.md、prompts/ddaigc3.md，每次调用只执行一轮，不得把三轮合并执行。不要把本 skill 用于普通润色、摘要压缩、翻译、查重降重、自由重写、逻辑审稿，或仅运行 scripts/run_aigc_round.py dry-run 这类脚本检查任务。'
user-invocable: true
---
...
- 多轮降 AIGC 必须严格按顺序执行三轮改写，但每次调用本 skill 只执行其中一轮。
- 三轮顺序固定为：`prompts/ddaigc1.md` -> `prompts/ddaigc2.md` -> `prompts/ddaigc3.md`，禁止跳轮或逆序。
- 每一轮的输出，必须作为下一轮的输入，轮次之间通过“降 AIGC 记录”在多个对话中串联。
- 在开始本轮改写之前，必须先读取“降 AIGC 记录”，根据当前文档已完成轮次自动选择本轮应使用的 prompt；如果记录中没有该文档，则默认本轮为第 1 轮，使用 `prompts/ddaigc1.md`。
...
为了避免单次对话上下文过长，同时在多个对话之间保持三轮顺序的一致性，本 skill 需要维护一份“降 AIGC 记录”，用于跨对话记住每篇文档已经执行到第几轮、使用了哪个 prompt，以及对应输出文件路径。
...
- 建议将记录写入工作区根目录下的 `finish/` 目录，例如 `finish/aigc_records.json`，格式可以采用 JSON 或其他结构化文本，但必须满足以下信息可恢复：
  ...
  - 每一轮使用的 prompt 文件路径（如 `prompts/ddaigc1.md` 等）。
  - 每一轮生成的输出文件路径（通常位于 `finish/intermediate/` 或 `finish/`）。
  - 每一轮采用的分段上限、分段数量，以及分段结构清单路径。
  - 可选：时间戳、执行环境等元信息。
- 在每次调用本 skill、开始改写之前，必须先读取这份记录：
  - 如果当前文档不存在任何记录，则本次默认执行第 1 轮，使用 `prompts/ddaigc1.md`。
  - 如果当前文档在记录中已经完成第 1 轮但未完成第 2 轮，则本次执行第 2 轮，使用 `prompts/ddaigc2.md`，并以上一轮输出的文件作为当前输入。
  - 如果当前文档已经完成第 1 轮和第 2 轮但未完成第 3 轮，则本次执行第 3 轮，使用 `prompts/ddaigc3.md`，并以上一轮输出的文件作为当前输入。
  - 如果当前文档已经完成三轮，则默认不再继续执行新的降 AIGC 轮次，而是提示用户该文档已完成 3 轮改写，如需特殊处理需用户明确确认。
...
- 在完成一轮降 AIGC 后更新记录（skill 的逻辑行为应当等价于执行以下命令）：
  ...
  - 其中：
    ...
    - `<round>` 为 1、2 或 3。
    - `<prompt>` 为本轮使用的 prompt 文件路径，例如 `prompts/ddaigc1.md`。
...
- 如果 `origin/` 中不存在对应原始文件：先要求用户上传文件，或提示用户先将原始文件放入当前工作区的 `origin/` 目录，再继续执行。
...
本 skill 的整体目标仍然是完成三轮顺序降 AIGC，但为了控制单次对话的上下文长度，**每次调用本 skill 只执行其中一轮**。三轮之间通过“降 AIGC 记录”和中间文件在多个对话中串联。
...
`确认 origin/ 中存在对应输入 docx -> 用外部工具将 docx 提取为纯文本 -> 第 1 轮：在第一个对话中调用本 skill，对提取出的文本执行第 1 轮改写并写入 finish/intermediate/ + 第 1 轮评分 + 更新降 AIGC 记录 -> 用户新开对话 -> 第 2 轮：在新对话中调用本 skill，以第 1 轮结果为输入执行第 2 轮改写并写入 finish/intermediate/ + 第 2 轮评分 + 更新降 AIGC 记录 -> 用户再次新开对话 -> 第 3 轮：在新对话中调用本 skill，以第 2 轮结果为输入执行第 3 轮改写并写入 finish/intermediate/ + 第 3 轮评分 + 更新降 AIGC 记录 -> 使用最终文本按 checklist.md 做终检 -> 将最终文本交给外部工具写出为新的 docx 到 finish/ 目录`
...
当“降 AIGC 记录”中尚未存在当前文档的记录时，默认本次执行第 1 轮。读取 skill 资源中的 `prompts/ddaigc1.md`。
...
当“降 AIGC 记录”中显示当前文档已完成第 1 轮但尚未完成第 2 轮时，本次执行第 2 轮。读取 skill 资源中的 `prompts/ddaigc2.md`。
...
当“降 AIGC 记录”中显示当前文档已完成第 1 轮和第 2 轮但尚未完成第 3 轮时，本次执行第 3 轮。读取 skill 资源中的 `prompts/ddaigc3.md`。
...
在“降 AIGC 记录”中显示某一文档已完成三轮顺序改写并对每一轮分别评分之后，再进行一次最终终检：
...
- 对于已完成的文档，已通过多次对话完成 3 轮顺序处理，每次调用本 skill 只执行一轮。
...
- 每次调用本 skill 只执行一轮降 AIGC，通过“降 AIGC 记录”在多次对话中按 1 -> 2 -> 3 顺序推进。
- 对于文本直接输入场景，如果这是某文档的第 3 轮或用户明确要求完成全部三轮并终检，则在本次对话中交付终稿、简要修改总结和评分。
