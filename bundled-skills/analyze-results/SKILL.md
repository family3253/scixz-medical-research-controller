...
description: Analyze ML experiment results, compute statistics, generate comparison tables and insights. Use when the user explicitly needs result comparison or interpretation as the main bottleneck. Prefer `academic-write-all-skill` as the first academic intake when the request is broad or mixed; use this skill as the dedicated downstream analysis owner before AWAS routes to report writing or manuscript prose.
