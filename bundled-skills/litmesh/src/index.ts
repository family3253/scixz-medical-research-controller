...
function assertPositiveInteger(field: string, value: number): void {
  if (!Number.isInteger(value) || value < 1) throw new Error(`litmesh: ${field} must be a positive integer`)
}

function assertNonNegativeInteger(field: string, value: number): void {
  if (!Number.isInteger(value) || value < 0) throw new Error(`litmesh: ${field} must be a non-negative integer`)
}
...
export function apply(ctx: Context, config: Config): void {
  ...
  const mode: Runtime['mode'] = resolved.mode
  ...
  if (!Number.isFinite(resolved.promptOrder)) throw new Error('litmesh: promptOrder must be a finite number')
  let baseUrl: string
  try {
  ...
  // always fails.
  if (resolved.sciDraw && !direct) applySciDrawTool(ctx, runtime)
  if (resolved.creditsTool || resolved.command || resolved.balanceRoute) applyCreditsTools(ctx, runtime, resolved.command, resolved.creditsTool && !direct, resolved.balanceRoute)
  ...
  if (resolved.promptGuidance) {
    ...
    if (text !== undefined) ctx.systemPrompt.section({ name: 'tool:litmesh', order: resolved.promptOrder, text })
  }
}
