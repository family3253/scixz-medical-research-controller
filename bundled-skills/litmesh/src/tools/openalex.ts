...
function abstractFromInverted(value: unknown): string | undefined {
  if (!isRecord(value)) return undefined
  const positions: Array<{ word: string; index: number }> = []
  for (const [word, slots] of Object.entries(value)) {
    if (Array.isArray(slots)) {
      for (const slot of slots) {
        if (typeof slot === 'number' && Number.isInteger(slot)) positions.push({ word, index: slot })
      }
    }
  }
 ...
 */
export function normalizeOpenalexWork(record: unknown): Paper | undefined {
 ...
 */
export async function runOpenalexSearch(runtime: Runtime, params: OpenalexSearchParams, signal: AbortSignal | undefined): Promise<PaperSearchValue> {
  ...
  const res = await runtime.client.fetchJson<{ results?: unknown[]; meta?: Record<string, unknown> }>(`${OPENALEX_API}?${query.toString()}`, { signal })
  if (!res.ok) throw new Error(`OpenAlex search failed: ${res.error}`)
  const papers = (res.data.results ?? []).map(normalizeOpenalexWork).filter((p): p is Paper => p !== undefined)
