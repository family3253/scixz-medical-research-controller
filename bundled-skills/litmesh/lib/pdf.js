 ...
 */
export async function fetchPdf(client, url, options) {
...
export function pdfCandidatesFor(landingUrl) {
    const candidates = [];
    let u;
    try {
    ...
    if (host.includes('sciencedirect.com'))
        candidates.push(`${u.origin}${path}/pdfft?isDTMRedir=true&download=true`);
    if (host.includes('springer.com') || host.includes('nature.com')) {
        const m = /\/articles?\/(10\.\d+\/[^\s?#]+)/.exec(path);
        if (m !== null)
            candidates.push(`https://${host}/content/pdf/${m[1]}.pdf`);
    }
    if (host.includes('onlinelibrary.wiley.com'))
        candidates.push(`${u.origin}${path.replace('/doi/', '/doi/pdfdirect/')}`);
    if (host.includes('tandfonline.com'))
        candidates.push(`${u.origin}${path.replace('/doi/full/', '/doi/pdf/').replace('/doi/abs/', '/doi/pdf/')}`);
    if (host.includes('mdpi.com') && path.endsWith('/htm'))
        candidates.push(`${u.origin}${path.replace(/\/htm$/, '/pdf')}`);
    if (host.includes('ieeexplore.ieee.org')) {
        const arnumber = /\/document\/(\d+)/.exec(path)?.[1];
        if (arnumber !== undefined)
            candidates.push(`https://ieeexplore.ieee.org/stampPDF/getPDF.jsp?tp=&arnumber=${arnumber}`);
    }
    if (host.includes('dl.acm.org') || host.includes('pubs.acs.org'))
        candidates.push(`${u.origin}${path.replace('/doi/', '/doi/pdf/')}`);
    if (host.includes('pubs.rsc.org'))
        candidates.push(`${u.origin}${path.replace('/articlelanding/', '/articlepdf/')}`);
    if (host.includes('journals.plos.org')) {
        const id = u.searchParams.get('id');
        if (id !== null)
            candidates.push(`${u.origin}${path.replace(/\/article$/, '/article/file')}?id=${encodeURIComponent(id)}&type=printable`);
    }
    if (host.includes('frontiersin.org') && /\/articles\/10\./.test(path))
        candidates.push(`${u.origin}${path.replace(/\/full$/, '')}/pdf`);
    if ((host.includes('biorxiv.org') || host.includes('medrxiv.org')) && path.startsWith('/content/'))
        candidates.push(`${u.origin}${path.replace(/(\.full)?(\.pdf)?$/, '')}.full.pdf`);
    if (host.includes('arxiv.org') && path.startsWith('/abs/'))
        candidates.push(`https://arxiv.org/pdf/${path.slice('/abs/'.length)}`);
    // Generic fallback for path-only URLs; a query string means the site keys on parameters, not paths.
    if (!path.endsWith('.pdf') && u.search.length === 0)
        candidates.push(`${u.origin}${path}.pdf`);
 ...
 */
export async function resolveDoiToPdf(client, doi, options) {
        ...
        if (res.ok) {
            ...
            if (contentType.includes('pdf') || contentType.includes('octet-stream')) {
                ...
                if (looksLikePdf(data))
                    return { ok: true, data, finalUrl: landingUrl, landingUrl };
            }
        }
 ...
 */
export function sliceText(full, offset, maxChars) {
...
export function presentReadCall(label, id) {
...
export function withCredits(value, credits) {
