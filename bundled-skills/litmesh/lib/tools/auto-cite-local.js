...
export function splitSentences(text) {
    ...
    if (tail.length > 0)
        out.push({ start: text.indexOf(tail, consumed), end: text.indexOf(tail, consumed) + tail.length, text: tail });
    return out;
}
...
export function sentenceKeywords(sentence, field, max = 6) {
...
export function matchScore(sentence, paper) {
...
export function pickAutoPoints(text, wanted) {
...
export function pickManualPoints(text) {
    const points = [];
    const sentences = splitSentences(text);
    const re = /\[CITE\]/g;
    let m;
...
function authorAPA(display) {
    const { initials, surname } = nameParts(display);
    return initials.length > 0 ? `${surname}, ${initials}` : surname;
}
/** Format one author as `Surname FM` (Vancouver style). */
function authorVancouver(display) {
    const { initials, surname } = nameParts(display);
    return initials.length > 0 ? `${surname} ${initials.replace(/[. ]/g, '')}` : surname;
}
function listIEEE(authors) {
    if (authors.length === 0)
        return 'Unknown authors';
    if (authors.length === 1)
        return authorIEEE(authors[0]);
    if (authors.length === 2)
        return `${authorIEEE(authors[0])} and ${authorIEEE(authors[1])}`;
    if (authors.length <= 6)
        return `${authors.slice(0, -1).map(authorIEEE).join(', ')}, and ${authorIEEE(authors[authors.length - 1])}`;
    return `${authorIEEE(authors[0])} et al.`;
}
function listAPA(authors) {
    if (authors.length === 0)
        return 'Unknown authors';
...
function listVancouver(authors) {
    if (authors.length === 0)
        return 'Unknown authors';
    if (authors.length <= 6)
        return authors.map(authorVancouver).join(', ');
 ...
 */
export function formatReference(style, paper, number) {
...
export function bibtexEntry(paper, number) {
    ...
    const lines = [`@article{ref${number},`];
    lines.push(`  title = {${bibtexEscape(paper.title)}}`);
    if (author.length > 0)
        lines.push(`  author = {${bibtexEscape(author)}}`);
    if (paper.venue !== undefined)
        lines.push(`  journal = {${bibtexEscape(paper.venue)}}`);
 ...
 */
export async function runLocalAutoCite(runtime, args, signal, agent) {
    const started = Date.now();
    const style = args.citationStyle ?? 'ieee';
    ...
    const chosen = new Array(points.length).fill(undefined);
    const seen = new Map(); // identity -> paper, for reuse across points
    let searches = 0;
    ...
    for (let i = 0; i < points.length; i++) {
        ...
        if (query.trim().length === 0)
            continue;
        if (i > 0)
            await delay(SEARCH_PACE_MS);
        const candidates = await searchOnce(query);
        let best;
        for (const paper of candidates) {
            if (args.excludePreprints === true && isPreprint(paper))
                continue;
            if (args.excludeConferences === true && isConference(paper))
                continue;
            let score = matchScore(point.sentence, paper);
            if (args.yearPreference !== undefined && paper.year !== undefined)
                score -= Math.min(Math.abs(paper.year - args.yearPreference) * 0.02, 0.2);
            if (args.preferredVenues !== undefined) {
                const venue = paper.venue?.toLowerCase() ?? '';
                if (args.preferredVenues.some((v) => venue.includes(v.toLowerCase())))
                    score += 0.25;
            }
            if (score >= 0.3 && (best === undefined || score > best.score))
                best = { paper, score };
        }
        if (best !== undefined) {
            ...
            if (reused !== undefined)
                chosen[i] = { point, paper: reused, score: matchScore(point.sentence, reused) };
        }
    }
