...
export function normalizeDoi(input) {
    ...
    if (!/^10\.\d{4,9}\/\S+$/.test(trimmed))
        throw new Error(`"${input}" is not a DOI (expected e.g. 10.1038/s41586-021-03819-2)`);
    return trimmed;
}
/**
 ...
 */
export function applyDoiTools(ctx, runtime, fullText) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (!res.ok)
                throw new Error(`DOI download failed: ${res.error}${res.landingUrl !== undefined ? ` (landing page: ${res.landingUrl})` : ''}`);
