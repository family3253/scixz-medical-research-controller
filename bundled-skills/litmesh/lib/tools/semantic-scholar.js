...
import { CREDITS_SCHEMA, PAPER_SCHEMA, PAPER_SEARCH_OUTPUT_SCHEMA, clip, compact, creditsMeta, formatAuthors, formatCredits, formatPaper, int, isRecord, paperSearchMeta, presentGenericWithCredits, presentPaperSearchCall, presentPaperSearchResult, renderPaperDetail, renderPaperSearch, str, } from '../paper.js';
...
function authorNames(value) {
    return Array.isArray(value)
        ? value.flatMap((a) => (isRecord(a) ? [str(a, 'name')] : [])).filter((n) => n !== undefined)
        : [];
}
function externalIdsOf(value) {
    if (!isRecord(value))
        return undefined;
    const ids = Object.fromEntries(Object.entries(value).flatMap(([k, v]) => typeof v === 'string' || typeof v === 'number' ? [[k, String(v)]] : []));
 ...
 */
export function normalizeS2Paper(record, extra) {
    if (!isRecord(record))
        return undefined;
    const title = str(record, 'title');
    const paperId = str(record, 'paperId');
    if (title === undefined || paperId === undefined)
        return undefined;
...
function normalizeEdge(record, side) {
    if (!isRecord(record))
        return undefined;
    const contexts = Array.isArray(record.contexts) ? record.contexts.filter((c) => typeof c === 'string') : [];
    const intents = Array.isArray(record.intents) ? record.intents.filter((c) => typeof c === 'string') : [];
 ...
 */
export function normalizeS2Author(record) {
    if (!isRecord(record))
        return undefined;
    const authorId = str(record, 'authorId');
    const name = str(record, 'name');
    if (authorId === undefined || name === undefined)
        return undefined;
    const affiliations = Array.isArray(record.affiliations) ? record.affiliations.filter((a) => typeof a === 'string' && a.length > 0) : [];
    return compact({
        authorId,
        name,
        affiliations,
...
function formatAuthor(author, index) {
    const facts = [];
    if (author.affiliations.length > 0)
        facts.push(author.affiliations.slice(0, 3).join('; '));
    if (author.hIndex !== undefined)
        facts.push(`h-index ${author.hIndex}`);
    if (author.paperCount !== undefined)
        facts.push(`${author.paperCount} papers`);
    if (author.citationCount !== undefined)
        facts.push(`${author.citationCount} citations`);
    ...
    if (typeof orcid === 'string')
        ids.push(`ORCID: ${orcid}`);
    if (author.homepage !== undefined)
        ids.push(`homepage: ${author.homepage}`);
    return `${index}. **${link}**${facts.length > 0 ? `\n   ${facts.join(' · ')}` : ''}\n   ${ids.join(' · ')}`;
}
function renderAuthorList(value, heading) {
    const parts = [];
    if (value.authors.length === 0)
        parts.push(`${heading}: no authors found.`);
    else {
        ...
        if (value.truncated)
            parts.push(value.nextOffset !== undefined ? `More available; call again with offset=${value.nextOffset}.` : 'More available; raise max_results.');
    }
...
function normalizeSnippet(record) {
    if (!isRecord(record) || !isRecord(record.snippet) || !isRecord(record.paper))
        return undefined;
...
function formatSnippets(value, abstractMaxChars) {
    const parts = [];
    if (value.snippets.length === 0)
        parts.push(`No full-text snippets found for "${value.query}".`);
    else {
        const items = value.snippets.map((s, i) => {
            const heading = s.paper.url !== undefined ? `[${s.paper.title}](${s.paper.url})` : s.paper.title;
            const where = [s.kind, s.section].filter((x) => x !== undefined).join(' / ');
...
function listValue(query, papers, total, offset, credits, next) {
 ...
 */
export async function runSemanticSearch(runtime, params, signal, agent) {
    ...
    const res = await runtime.graph.get('/graph/v1/paper/search', {
        apiKey,
        signal,
    ...
    const papers = (res.data.data ?? []).map((p) => normalizeS2Paper(p)).filter((p) => p !== undefined);
 ...
 */
export function applySemanticScholarTools(ctx, runtime, fullText) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (year !== undefined && year.length > 0 && !YEAR_FILTER.test(year))
                throw new Error('year must look like "2019", "2016-2020", "2010-", or "-2015"');
            const apiKey = await runtime.s2ApiKey();
            const res = await client.get('/graph/v1/paper/search/bulk', {
                apiKey,
                signal: exec.signal,
            ...
            if (!res.ok)
                throw new Error(`Semantic Scholar bulk search failed: ${res.error}`);
            const page = (res.data.data ?? []).map((p) => normalizeS2Paper(p)).filter((p) => p !== undefined);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const apiKey = await runtime.s2ApiKey();
            const res = await client.get('/graph/v1/snippet/search', { apiKey, signal: exec.signal, query: { query, limit } });
            if (!res.ok)
                throw new Error(`Semantic Scholar snippet search failed: ${res.error}`);
            const snippets = (res.data.data ?? []).map(normalizeSnippet).filter((s) => s !== undefined);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            const query = requireQuery(args.query);
            const apiKey = await runtime.s2ApiKey();
            const res = await client.get('/graph/v1/paper/search/match', { apiKey, signal: exec.signal, query: { query, fields: PAPER_FIELDS } });
            // The Graph API answers 404 when nothing matches; that is a result, not a failure.
            if (!res.ok && res.status === 404)
                return { query, found: false };
            if (!res.ok)
                throw new Error(`Semantic Scholar title match failed: ${res.error}`);
            const first = res.data.data?.[0];
            const paper = normalizeS2Paper(first);
            const credits = runtime.creditsOf(res, exec.agent);
            if (paper === undefined)
                return withCredits({ query, found: false }, credits);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const apiKey = await runtime.s2ApiKey();
            const res = await client.get(`/graph/v1/paper/${encodeURIComponent(paperId)}`, { apiKey, signal: exec.signal, query: { fields: PAPER_FIELDS } });
            if (!res.ok)
                throw new Error(`Semantic Scholar paper lookup failed: ${res.error}`);
            const paper = normalizeS2Paper(res.data);
            if (paper === undefined)
                throw new Error(`Semantic Scholar returned no paper for "${paperId}"`);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const apiKey = await runtime.s2ApiKey();
            const res = await client.post('/graph/v1/paper/batch', { ids }, { apiKey, signal: exec.signal, query: { fields: PAPER_FIELDS } });
            if (!res.ok)
    ...
    for (const [name, side, endpoint, what] of [
        ...
        ctx.tools.register(defineTool({
            ...
            async execute(args, exec) {
                ...
                const apiKey = await runtime.s2ApiKey();
                const res = await client.get(`/graph/v1/paper/${encodeURIComponent(paperId)}/${endpoint}`, {
                    apiKey, signal: exec.signal, query: { limit, offset, fields: EDGE_FIELDS },
                });
                if (!res.ok)
                    throw new Error(`Semantic Scholar ${endpoint} lookup failed: ${res.error}`);
                const papers = (res.data.data ?? []).map((row) => normalizeEdge(row, side)).filter((p) => p !== undefined);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const offset = args.offset !== undefined && args.offset > 0 ? Math.trunc(args.offset) : 0;
            const apiKey = await runtime.s2ApiKey();
            const res = await client.get(`/graph/v1/paper/${encodeURIComponent(paperId)}/authors`, {
                apiKey, signal: exec.signal, query: { limit, offset, fields: AUTHOR_FIELDS },
            });
            if (!res.ok)
                throw new Error(`Semantic Scholar paper authors lookup failed: ${res.error}`);
            const authors = (res.data.data ?? []).map(normalizeS2Author).filter((a) => a !== undefined);
            const next = typeof res.data.next === 'number' ? res.data.next : undefined;
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const offset = args.offset !== undefined && args.offset > 0 ? Math.trunc(args.offset) : 0;
            const apiKey = await runtime.s2ApiKey();
            const res = await client.get('/graph/v1/author/search', {
                apiKey, signal: exec.signal, query: { query, limit, offset, fields: AUTHOR_FIELDS },
            });
            if (!res.ok)
                throw new Error(`Semantic Scholar author search failed: ${res.error}`);
            const authors = (res.data.data ?? []).map(normalizeS2Author).filter((a) => a !== undefined);
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => {
                ...
                if (value.author !== undefined)
                    parts.push(formatAuthor(value.author, 1).replace(/^1\. /, ''));
                const credits = formatCredits(value.credits);
                if (credits !== undefined)
                    parts.push(credits);
                return [{ type: 'text', text: parts.join('\n\n') }];
        ...
        async execute(args, exec) {
            ...
            const apiKey = await runtime.s2ApiKey();
            const res = await client.get(`/graph/v1/author/${encodeURIComponent(authorId)}`, { apiKey, signal: exec.signal, query: { fields: AUTHOR_FIELDS } });
            if (!res.ok)
                throw new Error(`Semantic Scholar author lookup failed: ${res.error}`);
            const author = normalizeS2Author(res.data);
            if (author === undefined)
                throw new Error(`Semantic Scholar returned no author for "${authorId}"`);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const apiKey = await runtime.s2ApiKey();
            const res = await client.post('/graph/v1/author/batch', { ids }, { apiKey, signal: exec.signal, query: { fields: AUTHOR_FIELDS } });
            if (!res.ok)
                throw new Error(`Semantic Scholar author batch lookup failed: ${res.error}`);
            const authors = (Array.isArray(res.data) ? res.data : []).map(normalizeS2Author).filter((a) => a !== undefined);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const offset = args.offset !== undefined && args.offset > 0 ? Math.trunc(args.offset) : 0;
            const apiKey = await runtime.s2ApiKey();
            const res = await client.get(`/graph/v1/author/${encodeURIComponent(authorId)}/papers`, {
                apiKey, signal: exec.signal, query: { limit, offset, fields: PAPER_FIELDS },
            });
            if (!res.ok)
                throw new Error(`Semantic Scholar author papers lookup failed: ${res.error}`);
            const papers = (res.data.data ?? []).map((p) => normalizeS2Paper(p)).filter((p) => p !== undefined);
            const next = typeof res.data.next === 'number' ? res.data.next : undefined;
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const apiKey = await runtime.s2ApiKey();
            const res = await client.post('/recommendations/v1/papers/', { positivePaperIds: positive, negativePaperIds: negative }, {
                apiKey, signal: exec.signal, query: { limit, fields: PAPER_FIELDS },
            });
            if (!res.ok)
                throw new Error(`Semantic Scholar recommendations failed: ${res.error}`);
            const papers = (res.data.recommendedPapers ?? []).map((p) => normalizeS2Paper(p)).filter((p) => p !== undefined);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const limit = boundResults(args.max_results, limits, S2_RECOMMENDATION_CAP);
            const apiKey = await runtime.s2ApiKey();
            const res = await client.get(`/recommendations/v1/papers/forpaper/${encodeURIComponent(paperId)}`, {
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => {
                ...
                if (value.url !== undefined)
                    lines.push(`Paper page: ${value.url}`);
                const credits = formatCredits(value.credits);
                if (credits !== undefined)
                    lines.push('', credits);
        ...
        async execute(args, exec) {
            const paperId = requireQuery(args.paper_id, 'paper_id');
            const apiKey = await runtime.s2ApiKey();
            const res = await client.get(`/graph/v1/paper/${encodeURIComponent(paperId)}`, { apiKey, signal: exec.signal, query: { fields: 'paperId,title,openAccessPdf,externalIds,url' } });
            if (!res.ok)
                throw new Error(`Semantic Scholar paper lookup failed: ${res.error}`);
            const paper = normalizeS2Paper(res.data);
            if (paper === undefined)
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            const paperId = requireQuery(args.paper_id, 'paper_id');
            const apiKey = await runtime.s2ApiKey();
            const res = await client.get(`/graph/v1/paper/${encodeURIComponent(paperId)}`, { apiKey, signal: exec.signal, query: { fields: 'paperId,title,openAccessPdf,externalIds' } });
            if (!res.ok)
                throw new Error(`Semantic Scholar paper lookup failed: ${res.error}`);
            const paper = normalizeS2Paper(res.data);
            if (paper === undefined)
            ...
            if (paper.pdfUrl === undefined)
                throw new Error(`no open-access PDF is listed for "${paperId}"; try read_arxiv_paper (if it has an arXiv id) or read_by_doi`);
            const pdf = await fetchPdf(client, paper.pdfUrl, { signal: exec.signal, timeoutMs: timeouts.pdf });
            if (!pdf.ok)
                throw new Error(`PDF download failed: ${pdf.error}`);
