...
function normalizeReference(record, index) {
    if (!isRecord(record))
        return undefined;
    ...
    const numberish = (v) => (typeof v === 'number' && Number.isFinite(v) ? v : undefined);
    return compact({
        number: num,
        formatted,
        title: str(record, 'title'),
        year: numberish(record.year) !== undefined ? Math.trunc(record.year) : undefined,
        doi: str(record, 'doi'),
        url: str(record, 'url'),
        venue: str(record, 'venue'),
        citedBy: numberish(record.citedBy) !== undefined ? Math.trunc(record.citedBy) : undefined,
 ...
 */
export function applyAutoCiteTool(ctx, runtime) {
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => {
                ...
                if (value.bibtex !== undefined && value.bibtex.length > 0)
                    parts.push('BibTeX:\n```bibtex\n' + value.bibtex + '\n```');
        ...
        async execute(args, exec) {
            ...
            if (text.length < MIN_TEXT)
                throw new Error(`text must be at least ${MIN_TEXT} characters (got ${text.length})`);
            if (text.length > MAX_TEXT)
                throw new Error(`text must be at most ${MAX_TEXT} characters (got ${text.length})`);
            if (args.mode === 'manual' && !text.includes('[CITE]'))
                throw new Error('manual mode needs at least one [CITE] marker in the text');
            if (runtime.direct) {
            ...
            if (args.mode !== undefined)
                body.mode = args.mode;
            if (args.min_citations !== undefined)
                body.minCitations = Math.min(Math.max(1, Math.trunc(args.min_citations)), 50);
            if (args.field !== undefined && args.field.trim().length > 0)
                body.field = args.field.trim();
            if (args.year_preference !== undefined)
                body.yearPreference = Math.trunc(args.year_preference);
            if (args.exclude_preprints !== undefined)
                body.excludePreprints = args.exclude_preprints;
            if (args.exclude_conferences !== undefined)
                body.excludeConferences = args.exclude_conferences;
            if (args.citation_style !== undefined)
                body.citationStyle = args.citation_style;
            if (args.preferred_venues !== undefined && args.preferred_venues.length > 0)
                body.preferredVenues = args.preferred_venues;
            const res = await client.postSse('/api/proxy/auto-cite', body, { apiKey, signal: exec.signal, timeoutMs: timeouts.generation });
            if (!res.ok)
                throw new Error(`Auto-cite failed: ${res.error}`);
            const data = res.data;
            const annotatedText = str(data, 'annotatedText') ?? text;
            const references = Array.isArray(data.references) ? data.references.map(normalizeReference).filter((r) => r !== undefined) : [];
