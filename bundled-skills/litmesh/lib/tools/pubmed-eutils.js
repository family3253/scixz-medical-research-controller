...
async function pacedFetch(client, url, options) {
    const now = Date.now();
 ...
 */
export async function eutilsSearch(client, term, options) {
    ...
    const ids = Array.isArray(result.idlist) ? result.idlist.filter((id) => typeof id === 'string') : [];
...
function parseArticle(xml) {
    ...
    if (pmidMatch === null || titleMatch === null)
        return undefined;
    const authors = [];
    ...
    while ((am = authorRe.exec(authorBlock)) !== null) {
        ...
        const name = collective !== undefined ? cleanXml(collective)
            : last !== undefined ? cleanXml([fore, last].filter((p) => p !== undefined).join(' ')) : undefined;
    ...
    while ((abm = abstractRe.exec(xml)) !== null) {
        ...
        if (text.length > 0)
            abstracts.push(label !== undefined && label.toUpperCase() !== 'UNLABELLED' ? `${label}: ${text}` : text);
    }
    ...
    const pubDate = pubYear !== undefined ? [pubYear, pubMonth].filter((p) => p !== undefined).join(' ') : medlineDate !== undefined ? cleanXml(medlineDate) : undefined;
    ...
    let mm;
    while ((mm = meshRe.exec(xml)) !== null)
        mesh.push(cleanXml(mm[1]));
    const keywords = [];
    const kwRe = /<Keyword[^>]*>([^<]+)<\/Keyword>/g;
    let km;
    while ((km = kwRe.exec(xml)) !== null)
        keywords.push(cleanXml(km[1]));
    return {
        pmid: pmidMatch[1],
 ...
 */
export async function eutilsFetchPapers(client, ids, signal) {
    if (ids.length === 0)
        return [];
    ...
    if (!res.ok)
        throw new Error(`PubMed lookup failed: ${res.error}`);
    const papers = [];
 ...
 */
export async function eutilsLinks(client, pmid, linkname, limit, signal) {
    ...
    if (!res.ok)
        throw new Error(`PubMed link lookup failed: ${res.error}`);
    let body;
    try {
        body = JSON.parse(res.data);
    }
