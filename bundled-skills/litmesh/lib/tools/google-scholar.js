...
import { PAPER_SEARCH_OUTPUT_SCHEMA, compact, int, isRecord, paperSearchMeta, presentPaperSearchCall, presentPaperSearchResult, renderPaperSearch, str, } from '../paper.js';
import { boundResults, requireQuery } from '../runtime.js';
import { runOpenalexSearch } from './openalex.js';
const SOURCE = 'google-scholar';
 ...
 */
export function normalizeScholarResult(record) {
    if (!isRecord(record))
        return undefined;
 ...
 */
export async function runScholarSearch(runtime, params, signal, agent) {
    ...
    while (papers.length < wanted && !exhausted) {
        ...
        if (params.yearFrom !== undefined)
            body.yearFrom = params.yearFrom;
        if (params.yearTo !== undefined)
            body.yearTo = params.yearTo;
        const res = await runtime.client.post('/google-scholar/v1/search', body, { apiKey, signal });
        if (!res.ok) {
            // A later page failing keeps the pages already fetched; the first page failing is the failure.
            if (papers.length === 0)
                throw new Error(`Google Scholar search failed: ${res.error}`);
        ...
        const batch = (res.data.results ?? []).map(normalizeScholarResult).filter((p) => p !== undefined);
        papers.push(...batch);
        const pageCount = typeof res.data.resultsCount === 'number' ? res.data.resultsCount : batch.length;
        if (batch.length === 0 || pageCount < GS_PAGE_SIZE)
            exhausted = true;
        page += 1;
    }
 ...
 */
export function applyGoogleScholarTools(ctx, runtime) {
