---
name: research-lit
description: Search and analyze research papers, find related work, summarize key ideas. Use when the user explicitly needs literature retrieval or paper understanding as the main bottleneck. Prefer `academic-write-all-skill` as the first academic intake when the request is broad or mixed; use this skill as the dedicated downstream literature-search path after AWAS has already fixed the route.
