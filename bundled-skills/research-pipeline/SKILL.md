...
description: "Full research pipeline: Workflow 1 (idea discovery) → implementation → Workflow 2 (auto review loop). Use when the user explicitly wants an end-to-end autonomous research lifecycle rather than a normal AWAS-routed academic workflow. Prefer `academic-write-all-skill` as the first academic intake when the request is broad or mixed; use this skill only when the user truly wants the full pipeline as the primary owner."
