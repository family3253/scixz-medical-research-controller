...
If a requested Skill has no readable `SKILL.md`, classify it as `skill_missing`. If its `SKILL.md` is present but a required executable/dependency is unavailable, classify it as `runtime_missing`; do not report that the Skill itself is uninstalled. Read `registry/scixz_bindings.json` and the runtime-specific `registry/runtime_bindings.json` before declaring a Skill or runtime unavailable. In either case, record the limitation, choose a verified installed fallback that satisfies the same output contract, or ask whether installation is desired when no adequate fallback exists.
