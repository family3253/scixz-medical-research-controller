...
For manuscript review, [PaperReview.ai](https://paperreview.ai/) is integrated as an optional, guarded parallel review branch. After one frozen intake and explicit authorization for the exact English PDF, SciXZ starts the local primary review and the provider upload/poll path concurrently. Neither branch reads the other. When both same-fingerprint artifacts are complete, a fresh synthesis sub-agent compares them, independently verifies manuscript evidence, preserves disagreements, and dispositions every canonical `PR-xx` external issue exactly once. Separate tables or supplements are fingerprinted as companion evidence; the report must disclose that PaperReview did not receive them. Repeated website tests compare substantive review-content fingerprints, and identical outputs count as one external signal rather than multiple reviewers. The default human-facing deliverables are full Chinese and English editorial-review Word reports containing the materials reviewed, decision, reviewer panel, P0/P1/P2/P3 findings, source attribution, required actions, acceptance criteria, adversarial stress test, author questions, dimension scores, and revision roadmap. Strict fusion details remain in appendices; the compact renderer is an optional machine-audit attachment, not a replacement for this structure. Email and access tokens remain in private local state and are never committed. See [`references/external_review_tools.md`](references/external_review_tools.md).
